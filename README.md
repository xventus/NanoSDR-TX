...
The system was not designed to communicate with PC software but for RTOS, from which control is performed. Python scripts are used for verification. 
...
**Known limitation:** OS Win spikes longer than 128 ms (USB events, etc.) still drain the FIFO → ZOH events. See TODO below. Use RTOS ;-)

**TODO: Bidirectional flow control** — add UART TX byte from FPGA to Python when FIFO drops below a threshold. Python waits for the go-ahead signal. Eliminates ZOH events completely. Requires: `almost_empty` output from `smart_fifo.v`, UART TX logic in `top.v`, software change in Python scripts.
...
python send_nbfm.py --port COM4 --freq 100000 --cw --loop

# 3. AM modulation — 1 kHz tone at 150 kHz carrier
python send_am.py --port COM4 --freq 150000 --tone 1000 --mod 0.5 --rate 32000 --loop
