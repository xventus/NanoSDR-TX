 ...
 * @brief Amplitude Modulation (AM) Transmitter script for FPGA I/Q Modulator
 * @version 0.1
 * @date 2026-02-18
 * 
...
def main():
    parser = argparse.ArgumentParser(description="AM Transmitter for FPGA")
    ...
    # 5. Stream — drift-corrected pacing, factor 0.97
    # Python sends at rate/0.97 = +3% faster than FPGA consumption.
    # FIFO (4096 entries = 128 ms) stays nearly full → absorbs up to 128 ms OS jitter.
    # Steady-state overflow: ~3% of samples dropped (vs 15% with old factor 0.85).
    # time.monotonic() drift correction: if one sleep runs long, next fires immediately.
    # 5. Stream — drift-corrected pacing with Windows 1 ms timer
    ...
    chunk_size = 256
    print("Streaming..." + (" (loop, Ctrl+C to stop)" if args.loop else ""))
